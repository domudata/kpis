...
def inject_custom_css():
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    @media(max-width:768px){.cr{grid-template-columns:repeat(2,1fr)}.mh{padding:8px 10px;gap:8px}.mh h1{font-size:18px}.mh .logo{height:35px;max-width:70px}.mh .db{font-size:11px;padding:2px 8px}.cg,.dgrid{grid-template-columns:1fr}.car{flex-wrap:wrap;gap:2px}.car .cal{width:100%;text-align:left;padding-right:0;margin-bottom:2px}.car .cab{flex:1 1 70%}.car .cav-out,.car .cav-tgt{flex:1 1 15%;min-width:40px}.gbr{flex-direction:column;align-items:flex-start;gap:4px}.gbr-l{width:100%;margin-bottom:2px}.gbr-g{width:100%;flex-wrap:wrap}.gbr-w{flex:1 1 45%}.gbr-v{flex:1 1 10%;min-width:40px}.tw{font-size:10px}.tw thead th,.tw tbody td{padding:3px 4px}.stl{font-size:13px}.stTabs [data-baseweb="tab"]{padding:8px 12px;font-size:15px}.stTabs [data-baseweb="tab"] span{font-size:16px !important}div[data-testid="stDataEditor"] table, div[data-testid="stDataEditor"] th, div[data-testid="stDataEditor"] td { font-size: 14px !important; }}
    @media print {section[data-testid="stSidebar"], header[data-testid="stHeader"], div[data-testid="stToolbar"], div[data-testid="stHeaderActionElements"], footer, .stDeployButton, #MainMenu { display: none !important; }.main .block-container { padding-top: 0 !important; padding-left: 0 !important; padding-right: 0 !important; max-width: 100% !important; }.stButton, .stDownloadButton { display: none !important; }* { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }.tw, .synth-tbl { page-break-inside: avoid; overflow: visible !important; }}
...
def main():
    ...
    def html_statut_pivot(piv_df, table_class):
        ...
        for poste,row in piv_df.iterrows():
            ...
            for c in ["CRÉÉ","LANC","CLOT","TCLO"]: h+='<td style="text-align:center;%s">%d</td>'%(statut_colors[c], int(row.get(c,0)))
        ...
        for c in ["CRÉÉ","LANC","CLOT","TCLO"]: h+='<td style="text-align:center;font-weight:800;%s">%d</td>'%(statut_colors[c], int(piv_df[c].sum()))
    ...
    def calc_kpis(df_i, av_i, now_ts, posts):
        ...
        # DICTIONNAIRE POUR LES ANOMALIES (VRAIS COMPTAGES AVEC FILTRES)
        anom = {}
        ...
        anom["TAUX_REALISATION_CORRECTIF/PT"] = an["CRÉÉ"] + an["LANC"] # Anomalie = Non clôturés
        ...
        anom["OT préparation <1 mois"] = pr["Total"] - pr["<1 mois"]
        anom["OT préparation >3 mois"] = pr[">3 mois"]
        anom["OT préparation 1mois< <3mois"] = pr["1 mois < <3 mois"]
        ...
        anom["OT planification <1 mois"] = pl["Total"] - pl["<1 mois"]
        anom["OT planification >3 mois"] = pl[">3 mois"]
        anom["OT planification 1mois< <3mois"] = pl["1 mois < <3 mois"]
        ...
        anom["OT exécution <1 mois"] = ex["Total"] - ex["<1 mois"]
        anom["OT exécution >3 mois"] = ex[">3 mois"]
        anom["OT exécution 1mois< <3mois"] = ex["1 mois < <3 mois"]
        
        # 5. OT LANC ESTIME
        la=pd.pivot_table(df[df["Statut OT"]=="LANC"],index="Poste travail princ.",columns="OT LANC ESTIME",values="Ordre",aggfunc="count",fill_value=0).reindex(posts,fill_value=0)
        for c in ["OUI","NON"]: la[c]=la.get(c,0)
        la["Total"]=la["OUI"]+la["NON"]; la["OT LANC ESTIME"]=ckpi(la["OUI"],la["Total"])
        anom["OT LANC ESTIME"] = la["NON"]
        
        # 6. BACKLOG PRÉPARATION
        pc=pd.pivot_table(df[df["Statut OT"]=="CRÉÉ"],index="Poste travail princ.",columns="Backlog preparation",values="Ordre",aggfunc="count",fill_value=0).reindex(posts,fill_value=0)
        for c in ["CARACTERISE","NON CARACTERISE"]: pc[c]=pc.get(c,0)
        pc["Total"]=pc["CARACTERISE"]+pc["NON CARACTERISE"]; pc["Backlog préparation caractérisé"]=ckpi(pc["CARACTERISE"],pc["Total"])
        anom["Backlog préparation caractérisé"] = pc["NON CARACTERISE"]
        
        # 7. BACKLOG PLANIFICATION
        ...
        anom["Backlog planification caractérisé"] = plc["NON CARACTERISE"]
        
        # 8. OT CONFIME & OT_COR_EGAL (CORRECTION DU BUG)
        filt_clos = df["Statut OT"].isin(["CLOT","TCLO"])
        pv_conf=pd.pivot_table(df[filt_clos],index="Poste travail princ.",columns="OT CONFIME",values="Ordre",aggfunc="count",fill_value=0).reindex(posts,fill_value=0)
        ...
        anom["OT CONFIME"] = pv_conf["NON"]

        pv_cor=pd.pivot_table(df[filt_clos],index="Poste travail princ.",columns="OT_COR_EGAL",values="Ordre",aggfunc="count",fill_value=0).reindex(posts,fill_value=0)
        for c in ["OUI","NON"]: pv_cor[c]=pv_cor.get(c,0)
        pv_cor["Total"]=pv_cor["OUI"]+pv_cor["NON"]; pv_cor["OT_COR_EGAL"]=ckpi(pv_cor["OUI"],pv_cor["Total"])
        res['ot_cor_egal']=pv_cor
        anom["OT_COR_EGAL"] = pv_cor["NON"]
        
        # 9. TAUX D'APPROBATION DES AVIS
        ...
        anom["Taux d'approbation des Avis"] = tca["Total"] - tca["APRV"]
        ...
        anom["Performance Graissage"] = (g_df["_d"] - g_df["_n"])
        ...
        anom["Performance Inspection"] = (ins_df["_d"] - ins_df["_n"])
        ...
        anom["Performance Appels Systématiques"] = (sys_df["_d"] - sys_df["_n"])

        fiab_s=pd.Series(100.0,index=posts); avpan_s=pd.Series(100.0,index=posts)
        anom["OT Fiabilité"] = pd.Series(0, index=posts)
        anom["Total Avis de Panne"] = pd.Series(0, index=posts)

        # SAUVEGARDE DU DATAFRAME ANOMALIES
    ...
    # FONCTIONS POUR LES TABLEAUX HTML
    ...
    with st.sidebar:
        ...
        f_ot = st.file_uploader("Fichier OT (Excel)", type=["xlsx","xls"], key="ot_up")
        f_av = st.file_uploader("Fichier Avis (Excel)", type=["xlsx","xls"], key="av_up")
        
        if f_ot and f_av:
            try:
                df_raw, av_raw, all_posts, now_ts = prepare_data(f_ot.read(), f_av.read(), fichier_date)
                st.success(f"Données chargées : {len(df_raw)} OTs, {len(av_raw)} Avis")
                ...
                if posts_filt:
                    ...
                    ckdf = kdata['ckdf']
                    pkdf = kdata['pkdf']
                    anom_df = kdata['anom'] # Source unique des anomalies
                    ...
                    with tab1:
                        ...
                        if is_quality_view:
                            kpi_df_show = ckdf[QK + ["Score Qualité"]]
                            kpi_list_show = QK
                            t_class = "qt"
                            c1_val, c1_lab = ckdf["Score Qualité"].mean(), "Score Qualité Moyen"
                            c2_val, c2_lab = ckdf["TAUX_REALISATION_CORRECTIF/PT"].mean(), "Taux Réal. Correctif"
                            c3_val, c3_lab = anom_df[QK].sum().sum(), "Total Anomalies Qualité"
                            c4_val, c4_lab = len(posts_filt), "Postes Sélectionnés"
                        else:
                            kpi_df_show = pkdf[PK + ["Score Performance", "Score Qualité"]]
                            kpi_list_show = PK
                            t_class = "pt"
                            c1_val, c1_lab = pkdf["Score Performance"].mean(), "Score Performance Moyen"
                            c2_val, c2_lab = pkdf["Taux d'approbation des Avis"].mean(), "Taux Appr. Avis"
                            c3_val, c3_lab = anom_df[PK].sum().sum(), "Total Anomalies Perf"
                            c4_val, c4_lab = len(posts_filt), "Postes Sélectionnés"
                            
                        # Cartes
                        st.markdown(f'<div class="cr"><div class="cc c1"><div class="cv">{c1_val:.1f}</div><div class="cl">{c1_lab}</div></div><div class="cc c2"><div class="cv">{c2_val:.1f}%</div><div class="cl">{c2_lab}</div></div><div class="cc c4"><div class="cv">{c3_val}</div><div class="cl">{c3_lab}</div></div><div class="cc c5"><div class="cv">{c4_val}</div><div class="cl">{c4_lab}</div></div></div>', unsafe_allow_html=True)
                        
                        # Tableau KPI
                        st.markdown(f'<div class="stl">Tableau des Indicateurs ({vue.split(" ")[0]})</div>', unsafe_allow_html=True)
                        st.markdown(html_kpi_table(kpi_df_show, CIBLE, t_class), unsafe_allow_html=True)
                        
                        # NOUVEAU TABLEAU ANOMALIES DIRECTEMENT SOUS LES KPI
                        st.markdown('<div class="stl">Nombre d\'anomalies par KPI et Poste de travail</div>', unsafe_allow_html=True)
                        st.markdown(html_anom_table(anom_df, kpi_list_show, "anom"), unsafe_allow_html=True)
                        
                        # Graphiques (conservés intégralement)
                        st.markdown('<div class="stl">Analyses Graphiques</div>', unsafe_allow_html=True)
                        col_g1, col_g2 = st.columns(2)
                        with col_g1:
                            show_pie_pair(kdata['an_pivot'], "Correctifs (Global)")
                        with col_g2:
                    ...
                    # ================= TAB 2 : PLANS D'ACTIONS =================
                    with tab2:
                        st.markdown('<div class="stl">Plans d\'Actions (Source unique : Tableau des Anomalies)</div>', unsafe_allow_html=True)
                        ano_rows = []
                        all_kpis_for_action = QK + PK
                        for poste in posts_filt:
                            for kpi in all_kpis_for_action:
                                if kpi in ckdf.columns: val = ckdf.loc[poste, kpi]
                                elif kpi in pkdf.columns: val = pkdf.loc[poste, kpi]
                                else: continue
                    ...
                    with tab4:
                        ...
                        if st.button("Sauvegarder et Exporter les KPIs dans Excel", key="export_btn"):
                            ...
                            p_rows = df_to_rows(pkdf)
                            p_cols = ["Poste de travail"] + list(pkdf.columns)
                            q_rows = df_to_rows(ckdf)
                            q_cols = ["Poste de travail"] + list(ckdf.columns)
                            
                            # Génère ano_p_r et ano_q_r pour le export excel
                            ano_p_r, ano_q_r = [], []
                            for r in ano_rows:
            ...
            st.info("Veuillez charger les fichiers OT et Avis pour commencer.")
            st.markdown('<div class="mh"><h1>DASHBOARD KPI</h1><div class="db">En attente de fichiers</div></div>', unsafe_allow_html=True)
