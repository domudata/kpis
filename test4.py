...
def inject_custom_css():
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .cell-green{background:#d1fae5!important;color:#065f46!important;font-weight:700!important}
    .cell-red{background:#fee2e2!important;color:#991b1b!important;font-weight:700!important}
    .cell-yellow{background:#fef3c7!important;color:#92400e!important;font-weight:700!important}
    .tg-green{background:#10b981!important;color:#fff!important;font-weight:900!important;font-size:13px!important}
    .tg-red{background:#ef4444!important;color:#fff!important;font-weight:900!important;font-size:13px!important}
    ...
    .ano-detail-box .ano-title{
        font-weight:800;
        color:#856404;
        margin-bottom:6px;
        font-size:13px;
    }
    .ano-detail-box .ano-list{
        display:flex;
        flex-wrap:wrap;
        gap:4px;
    }
    .ano-detail-box .ano-item{
        background:#fff;
        border:1px solid #ffc107;
        border-radius:4px;
        padding:2px 8px;
...
# ============================================================
# FONCTION DE COLORATION DES CELLULES (REGLES MISES A JOUR)
# ============================================================
def get_cell_color(kpi_name, value):
    """Retourne 'green', 'red', ou 'grey' selon les regles definies."""
    try:
        v = float(value)
    except Exception:
        return "grey"
    
    if kpi_name == "TAUX_REALISATION_CORRECTIF/PT":
        return "green" if v >= 95 else "red"
    
    if kpi_name in ["OT préparation <1 mois", "OT planification <1 mois", "OT exécution <1 mois"]:
        return "green" if v >= 80 else "red"
    
    if kpi_name in ["OT préparation >3 mois", "OT planification >3 mois", "OT exécution >3 mois"]:
        return "green" if v <= 5 else "red"
    
    if kpi_name in ["OT préparation 1mois< <3mois", "OT planification 1mois< <3mois", "OT exécution 1mois< <3mois"]:
        return "green" if v <= 15 else "red"
    
    if kpi_name in ["Performance Graissage", "Performance Inspection"]:
        return "green" if v >= 95 else "red"
    
    if kpi_name == "Performance Appels Systématiques":
        return "green" if v >= 85 else "red"
    
    if kpi_name == "appel avis approuvé":
        return "green" if v >= 95 else "red"
    
    if kpi_name == "OT_COR_EGAL":
        return "green" if v >= 95 else "red"
    
    if kpi_name in ["OT LANC ESTIME", "Backlog préparation caractérisé", "Backlog planification caractérisé",
                     "OT CONFIME", "OT Fiabilité", "Total Avis de Panne"]:
        return "green" if v >= 100 else "red"
    
    return "grey"

def get_total_general_color(value):
    """Couleur pour Total General: >=90 vert, sinon rouge."""
    try:
        v = float(value)
    except Exception:
        return "red"
    return "green" if v >= 90 else "red"

def get_bar_color(kpi, val):
    """Couleur pour les barres de graphique."""
    try: v = float(val)
    except: return "#cbd5e0"
    c = get_cell_color(kpi, val)
...
def main():
    ...
    # ============================================================
    # CALCUL DES KPIs AVEC DETAILS DES ANOMALIES
    # ============================================================
    def calc_kpis(df_i, av_i, now_ts, posts):
        ...
        # ============================================================
        # CALCUL DES ANOMALIES : nombre d'OT/Avis a corriger pour 100%
        # ============================================================
        ano_details = {}  # (poste, kpi) -> list of OT/Avis numbers
        ano_counts = {}   # (poste, kpi) -> int count

        # TAUX_REALISATION_CORRECTIF/PT : OT correctifs non clôturés
        corr_non_closed = df[filt_corr & (~df["Statut OT"].isin(["CLOT","TCLO"]))]
        for poste in posts:
            ots = corr_non_closed[corr_non_closed["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"TAUX_REALISATION_CORRECTIF/PT")] = ots
            ano_counts[(poste,"TAUX_REALISATION_CORRECTIF/PT")] = len(ots)

        # OT préparation <1 mois : OT en CRÉÉ+CRPR avec age >= 1 mois
        prep_not_lt1 = df[(df["Statut OT"]=="CRÉÉ")&(df["Statut utilisateur"].str.contains("CRPR",na=False))&(df["amp"]>=1)]
        for poste in posts:
            ots = prep_not_lt1[prep_not_lt1["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"OT préparation <1 mois")] = ots
            ano_counts[(poste,"OT préparation <1 mois")] = len(ots)

        # OT préparation >3 mois : OT en CRÉÉ+CRPR avec age > 3 mois
        prep_gt3 = df[(df["Statut OT"]=="CRÉÉ")&(df["Statut utilisateur"].str.contains("CRPR",na=False))&(df["amp"]>3)]
        for poste in posts:
            ots = prep_gt3[prep_gt3["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"OT préparation >3 mois")] = ots
            ano_counts[(poste,"OT préparation >3 mois")] = len(ots)

        # OT préparation 1mois< <3mois : OT en CRÉÉ+CRPR avec 1 < age < 3
        prep_1_3 = df[(df["Statut OT"]=="CRÉÉ")&(df["Statut utilisateur"].str.contains("CRPR",na=False))&(df["amp"]>1)&(df["amp"]<3)]
        for poste in posts:
            ots = prep_1_3[prep_1_3["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"OT préparation 1mois< <3mois")] = ots
            ano_counts[(poste,"OT préparation 1mois< <3mois")] = len(ots)

        # OT planification <1 mois : OT en LANC+ATPL avec age >= 1 mois
        plan_not_lt1 = df[(df["Statut OT"]=="LANC")&(df["Statut utilisateur"].str.contains("ATPL",case=False,na=False))&(df["amlp"]>=1)]
        for poste in posts:
            ots = plan_not_lt1[plan_not_lt1["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"OT planification <1 mois")] = ots
            ano_counts[(poste,"OT planification <1 mois")] = len(ots)

        # OT planification >3 mois
        plan_gt3 = df[(df["Statut OT"]=="LANC")&(df["Statut utilisateur"].str.contains("ATPL",case=False,na=False))&(df["amlp"]>3)]
        for poste in posts:
            ots = plan_gt3[plan_gt3["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"OT planification >3 mois")] = ots
            ano_counts[(poste,"OT planification >3 mois")] = len(ots)

        # OT planification 1mois< <3mois
        plan_1_3 = df[(df["Statut OT"]=="LANC")&(df["Statut utilisateur"].str.contains("ATPL",case=False,na=False))&(df["amlp"]>1)&(df["amlp"]<3)]
        for poste in posts:
            ots = plan_1_3[plan_1_3["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"OT planification 1mois< <3mois")] = ots
            ano_counts[(poste,"OT planification 1mois< <3mois")] = len(ots)

        # OT exécution <1 mois : OT en LANC+SOPL avec age >= 1 mois
        exe_not_lt1 = df[(df["Statut OT"]=="LANC")&(df["Contient SOPL"]==1)&(df["amex"]>=1)]
        for poste in posts:
            ots = exe_not_lt1[exe_not_lt1["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"OT exécution <1 mois")] = ots
            ano_counts[(poste,"OT exécution <1 mois")] = len(ots)

        # OT exécution >3 mois
        exe_gt3 = df[(df["Statut OT"]=="LANC")&(df["Contient SOPL"]==1)&(df["amex"]>3)]
        for poste in posts:
            ots = exe_gt3[exe_gt3["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"OT exécution >3 mois")] = ots
            ano_counts[(poste,"OT exécution >3 mois")] = len(ots)

        # OT exécution 1mois< <3mois
        exe_1_3 = df[(df["Statut OT"]=="LANC")&(df["Contient SOPL"]==1)&(df["amex"]>1)&(df["amex"]<3)]
        for poste in posts:
            ots = exe_1_3[exe_1_3["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"OT exécution 1mois< <3mois")] = ots
            ano_counts[(poste,"OT exécution 1mois< <3mois")] = len(ots)

        # Performance Graissage : OT type 350 non clôturés
        g_ano = df[(df["Contient SOPL"]==1)&(df["_tw_num"]==350)&(~df["Statut OT"].isin(["CLOT","TCLO"]))]
        for poste in posts:
            ots = g_ano[g_ano["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"Performance Graissage")] = ots
            ano_counts[(poste,"Performance Graissage")] = len(ots)

        # Performance Inspection : OT type 290,300,310 non clôturés
        ins_ano = df[(df["Contient SOPL"]==1)&(df["_tw_num"].isin(ins_types))&(df["Date de début planifiée"].notna())&(df["Date de début planifiée"]<=now_ts)&(~df["Statut OT"].isin(["CLOT","TCLO"]))]
        for poste in posts:
            ots = ins_ano[ins_ano["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"Performance Inspection")] = ots
            ano_counts[(poste,"Performance Inspection")] = len(ots)

        # Performance Appels Systématiques : OT type 360 non clôturés
        sys_ano = df[(df["Contient SOPL"]==1)&(df["_tw_num"]==360)&(df["Date de début planifiée"].notna())&(df["Date de début planifiée"]<=now_ts)&(~df["Statut OT"].isin(["CLOT","TCLO"]))]
        for poste in posts:
            ots = sys_ano[sys_ano["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"Performance Appels Systématiques")] = ots
            ano_counts[(poste,"Performance Appels Systématiques")] = len(ots)

        # appel avis approuvé : Avis non approuvés (pas APRV)
        av_non_apprv = avf[~avf["Statut utilisateur"].str.contains("APRV",na=False)]
        for poste in posts:
            avs = av_non_apprv[av_non_apprv["Poste travail princ."]==poste]["Avis"].tolist()
            ano_details[(poste,"appel avis approuvé")] = avs
            ano_counts[(poste,"appel avis approuvé")] = len(avs)

        # OT LANC ESTIME : OT en LANC non estimés
        la_non = df[(df["Statut OT"]=="LANC")&(df["OT LANC ESTIME"]=="NON")]
        for poste in posts:
            ots = la_non[la_non["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"OT LANC ESTIME")] = ots
            ano_counts[(poste,"OT LANC ESTIME")] = len(ots)

        # Backlog préparation caractérisé : OT en CRÉÉ non caractérisés
        pc_non = df[(df["Statut OT"]=="CRÉÉ")&(df["Backlog preparation"]=="NON CARACTERISE")]
        for poste in posts:
            ots = pc_non[pc_non["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"Backlog préparation caractérisé")] = ots
            ano_counts[(poste,"Backlog préparation caractérisé")] = len(ots)

        # Backlog planification caractérisé : OT en LANC non caractérisés
        plc_non = df[(df["Statut OT"]=="LANC")&(df["Backlog planification"]=="NON CARACTERISE")]
        for poste in posts:
            ots = plc_non[plc_non["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"Backlog planification caractérisé")] = ots
            ano_counts[(poste,"Backlog planification caractérisé")] = len(ots)

        # OT CONFIME : OT non confirmés
        oc_non = df[df["OT CONFIME"]=="NON"]
        for poste in posts:
            ots = oc_non[oc_non["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"OT CONFIME")] = ots
            ano_counts[(poste,"OT CONFIME")] = len(ots)

        # OT_COR_EGAL : OT avec coûts non égaux
        oce_non = df[df["OT_COR_EGAL"]=="NON"]
        for poste in posts:
            ots = oce_non[oce_non["Poste travail princ."]==poste]["Ordre"].tolist()
            ano_details[(poste,"OT_COR_EGAL")] = ots
            ano_counts[(poste,"OT_COR_EGAL")] = len(ots)

        # OT Fiabilité et Total Avis de Panne : toujours 100%, 0 anomalies
        for poste in posts:
            ano_details[(poste,"OT Fiabilité")] = []
            ano_counts[(poste,"OT Fiabilité")] = 0
            ano_details[(poste,"Total Avis de Panne")] = []
            ano_counts[(poste,"Total Avis de Panne")] = 0

        res['ano_details'] = ano_details
        res['ano_counts'] = ano_counts
        return res

    # ============================================================
    # CONSTRUCTION DU TABLEAU HTML AVEC TOTAL GENERAL CORRIGE
    # ============================================================
    def build_kpi_table_html(ckdf, kpi_list, table_class, ano_counts_dict, posts):
        """Construit le tableau HTML avec coloration cellule par cellule et Total General."""
        cols_header = ["Poste de travail"] + kpi_list + ["Total général"]
        h = '<table class="tw %s"><thead><tr>' % table_class
        for c in cols_header:
            h += '<th>%s</th>' % c
        h += '</tr></thead><tbody>'

        # Pré-calcul des couleurs pour toutes les cellules
        cell_colors = {}  # (poste, kpi) -> 'green'/'red'/'grey'
        for poste in posts:
            for kpi in kpi_list:
                val = ckdf.loc[poste, kpi] if poste in ckdf.index and kpi in ckdf.columns else 0
                cell_colors[(poste, kpi)] = get_cell_color(kpi, val)
        ...
        for poste in posts:
            ...
            for kpi in kpi_list:
                val = ckdf.loc[poste, kpi] if poste in ckdf.index and kpi in ckdf.columns else 0
                color = cell_colors[(poste, kpi)]
                css_class = "cell-%s" % color
                if color in ["green", "yellow"]:
                    row_green_count += 1
                h += '<td class="%s" style="text-align:center">%.1f%%</td>' % (css_class, val)
            # Total général pour la ligne (poste)
            row_total = (row_green_count / len(kpi_list)) * 100 if len(kpi_list) > 0 else 0
            tg_color = get_total_general_color(row_total)
            tg_css = "tg-%s" % tg_color
            h += '<td class="%s" style="text-align:center">%.1f%%</td>' % (tg_css, row_total)
            h += '</tr>'

        # Ligne Total général (par colonne / indicateur)
        h += '<tr><td style="font-weight:900;background:#1e3a5f!important;color:#fff!important">Total général</td>'
        all_row_greens = 0
        total_cells = len(kpi_list) * len(posts)
        for kpi in kpi_list:
            col_green_count = 0
            for poste in posts:
                if cell_colors.get((poste, kpi)) in ["green", "yellow"]:
                    col_green_count += 1
                    all_row_greens += 1
            col_total = (col_green_count / len(posts)) * 100 if len(posts) > 0 else 0
            tg_color = get_total_general_color(col_total)
            tg_css = "tg-%s" % tg_color
            h += '<td class="%s" style="text-align:center">%.1f%%</td>' % (tg_css, col_total)
        # Total général de la ligne Total général (bas-droite)
        grand_total = (all_row_greens / total_cells) * 100 if total_cells > 0 else 0
        tg_color = get_total_general_color(grand_total)
        tg_css = "tg-%s" % tg_color
        h += '<td class="%s" style="text-align:center">%.1f%%</td>' % (tg_css, grand_total)
        h += '</tr></tbody></table>'
        return h
    ...
    with st.sidebar:
        ...
        date_input = st.text_input("Date (jj/mm/aaaa)", value=fichier_date, key="date_in")
        st.markdown("### 🔍 Filtres")
        filtre_poste = st.multiselect("Postes de travail", [], key="fp")
        
        # Bouton sauvegarde Excel
        if st.button("💾 Sauvegarder KPIs Excel", key="save_btn"):
    ...
    if not ot_file or not av_file:
        ...
        <h2 style="font-size:28px;font-weight:800;color:#1e3a5f">Chargement des données</h2>
        <p style="font-size:16px;margin-top:10px">Veuillez charger les fichiers OT et Avis depuis la barre latérale.</p>
        </div>""", unsafe_allow_html=True)
        st.stop()

    with st.spinner("Traitement des données..."):
        df_raw, av_raw, all_posts, now_ts = prepare_data(ot_file.read(), av_file.read(), date_input)
    
    posts = filtre_poste if filtre_poste else all_posts
    if not posts:
        st.markdown('<div class="es">Aucun poste de travail trouvé correspondant aux filtres.</div>', unsafe_allow_html=True)
        st.stop()

    res = calc_kpis(df_raw, av_raw, now_ts, posts)
    ckdf = res['ckdf']
    ano_details = res['ano_details']
    ano_counts = res['ano_counts']

    # ============================================================
    # EN-TETE
    # ============================================================
    logo_b64 = get_logo_base64()
    logo_html = '<img class="logo" src="data:image/png;base64,%s" alt="Logo">' % logo_b64 if logo_b64 else '<div style="width:50px;height:50px;background:rgba(255,255,255,.2);border-radius:6px;display:flex;align-items:center;justify-content:center;font-size:24px">⚙️</div>'
    st.markdown('<div class="mh">%s<h1>Dashboard KPI Maintenance</h1><div class="db">📅 %s</div></div>' % (logo_html, date_input), unsafe_allow_html=True)
    ...
    # Score Performance : Total General moyen des lignes pour QK
    perf_row_scores = []
    for poste in posts:
        green_count = sum(1 for kpi in QK if get_cell_color(kpi, ckdf.loc[poste, kpi]) in ["green", "yellow"])
        perf_row_scores.append((green_count / len(QK)) * 100 if QK else 0)
    score_perf = np.mean(perf_row_scores) if perf_row_scores else 0
    
    # Score Qualité : Total General moyen des lignes pour PK
    qual_row_scores = []
    for poste in posts:
        green_count = sum(1 for kpi in PK if get_cell_color(kpi, ckdf.loc[poste, kpi]) in ["green", "yellow"])
        qual_row_scores.append((green_count / len(PK)) * 100 if PK else 0)
    score_qual = np.mean(qual_row_scores) if qual_row_scores else 0

    # Nombre total anomalies
    total_ano = sum(ano_counts.values())

    # Nombre postes
    nb_postes = len(posts)

    cr = '<div class="cr">'
    cr += '<div class="cc c1"><div class="cv">%d</div><div class="cl">Total OT</div></div>' % total_ot
    cr += '<div class="cc c2"><div class="cv">%d</div><div class="cl">Total Avis</div></div>' % total_av
    cr += '<div class="cc c3"><div class="cv">%.1f%%</div><div class="cl">Score Performance</div></div>' % score_perf
    cr += '<div class="cc c5"><div class="cv">%.1f%%</div><div class="cl">Score Qualité</div></div>' % score_qual
    cr += '<div class="cc c4"><div class="cv">%d</div><div class="cl">Anomalies</div></div>' % total_ano
    cr += '<div class="cc c6"><div class="cv">%d</div><div class="cl">Postes</div></div>' % nb_postes
    cr += '<div class="cc c7"><div class="cv">%d</div><div class="cl">Indicateurs</div></div>' % len(ALL_KPI)
    cr += '<div class="cc c8"><div class="cv">%.1f%%</div><div class="cl">Score Global</div></div>' % ((score_perf + score_qual) / 2)
    cr += '</div>'
    ...
    # ONGLETS
    # ============================================================
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["📊 Synthèse", "⚡ Performance", "✅ Qualité", "📋 Plan d'action", "🔍 Anomalies"])
    ...
    with tab1:
        ...
        synth_html = build_kpi_table_html(ckdf, ALL_KPI, "st", ano_counts, posts)
        st.markdown(synth_html, unsafe_allow_html=True)

        # Graphiques résumé
        col_g1, col_g2 = st.columns(2)
        with col_g1:
            st.markdown('<div class="stl">Distribution Scores Performance par Poste</div>', unsafe_allow_html=True)
            if posts:
                fig_perf = go.Figure()
                fig_perf.add_trace(go.Bar(
                    x=posts, y=perf_row_scores,
                    marker_color=['#10b981' if s >= 90 else '#ef4444' for s in perf_row_scores],
                    text=[f'{s:.1f}%' for s in perf_row_scores],
                    textposition='outside',
                    textfont=dict(size=11, color='#1e293b')
                ))
                fig_perf.add_hline(y=90, line_dash="dash", line_color="#1e3a5f", annotation_text="Seuil 90%")
                fig_perf.update_layout(height=400, margin=dict(t=40, b=80, l=40, r=20),
                                       xaxis_tickangle=-45, yaxis_range=[0, 110],
                                       plot_bgcolor='rgba(0,0,0,0)')
                st.plotly_chart(fig_perf, use_container_width=True)
        
        with col_g2:
            st.markdown('<div class="stl">Distribution Scores Qualité par Poste</div>', unsafe_allow_html=True)
            if posts:
                fig_qual = go.Figure()
                fig_qual.add_trace(go.Bar(
                    x=posts, y=qual_row_scores,
                    marker_color=['#10b981' if s >= 90 else '#ef4444' for s in qual_row_scores],
                    text=[f'{s:.1f}%' for s in qual_row_scores],
                    textposition='outside',
                    textfont=dict(size=11, color='#1e293b')
                ))
                fig_qual.add_hline(y=90, line_dash="dash", line_color="#1e3a5f", annotation_text="Seuil 90%")
                fig_qual.update_layout(height=400, margin=dict(t=40, b=80, l=40, r=20),
                                       xaxis_tickangle=-45, yaxis_range=[0, 110],
                                       plot_bgcolor='rgba(0,0,0,0)')
    ...
    with tab2:
        ...
        perf_html = build_kpi_table_html(ckdf, QK, "pt", ano_counts, posts)
        st.markdown(perf_html, unsafe_allow_html=True)

        # Barres horizontales par indicateur
        st.markdown('<div class="stl">Détail par Indicateur de Performance</div>', unsafe_allow_html=True)
        for kpi in QK:
            cible_val = CIBLE.get(kpi, 100)
            is_lower = kpi in LOWER_BETTER
            vals = [ckdf.loc[p, kpi] for p in posts]
            colors_bar = [get_bar_color(kpi, v) for v in vals]
            
            ca_html = '<div class="ca"><div class="ct">%s <span style="float:right;font-size:11px;color:#64748b">Cible: %s%s</span></div>' % (kpi, '<=' if is_lower else '>=', cible_val)
            for i, poste in enumerate(posts):
                v = vals[i]
                bar_w = min(v, 120)
                cible_pos = (cible_val / 120) * 100
                ca_html += '<div class="car"><div class="cal">%s</div><div class="cab"><div class="caf" style="width:%.1f%%;background:%s"></div><div class="target-mark" style="left:%.1f%%"></div></div><div class="cav-out">%.1f%%</div></div>' % (poste, bar_w, colors_bar[i], cible_pos, v)
            ca_html += '</div>'
    ...
    with tab3:
        ...
        qual_html = build_kpi_table_html(ckdf, PK, "qt", ano_counts, posts)
        st.markdown(qual_html, unsafe_allow_html=True)

        # Barres horizontales par indicateur
        st.markdown('<div class="stl">Détail par Indicateur de Qualité</div>', unsafe_allow_html=True)
        for kpi in PK:
            cible_val = CIBLE.get(kpi, 100)
            is_lower = kpi in LOWER_BETTER
            vals = [ckdf.loc[p, kpi] for p in posts]
    ...
    # -----------------------------------------------------------
    with tab4:
        st.markdown('<div class="stl">Plan d\'Action — Anomalies à Corriger</div>', unsafe_allow_html=True)
        st.markdown('<p style="font-size:13px;color:#64748b;margin-bottom:12px">Cliquez sur le nombre d\'anomalies pour voir le détail des OT/Avis concernés.</p>', unsafe_allow_html=True)
        
        # Regrouper les anomalies par KPI
        kpi_ano_summary = []
        for kpi in ALL_KPI:
            total_for_kpi = 0
            poste_details = []
            for poste in posts:
                cnt = ano_counts.get((poste, kpi), 0)
                if cnt > 0:
                    total_for_kpi += cnt
                    poste_details.append((poste, cnt))
            if total_for_kpi > 0:
                kpi_ano_summary.append((kpi, total_for_kpi, poste_details))
        
        kpi_ano_summary.sort(key=lambda x: x[1], reverse=True)
        
        if not kpi_ano_summary:
            st.markdown('<div class="es" style="background:#d1fae5;color:#065f46;border-radius:8px;padding:20px">✅ Aucune anomalie détectée — Tous les indicateurs sont au vert !</div>', unsafe_allow_html=True)
        else:
            # Tableau récapitulatif
            pa_h = '<table class="plan-action-table"><thead><tr>'
            pa_h += '<th>Indicateur</th><th>Responsable</th><th>Action</th><th>Nb Anomalies</th>'
            pa_h += '</tr></thead><tbody>'
            for kpi, total, pdets in kpi_ano_summary:
                resp = KPI_RESP_MAP.get(kpi, "-")
                action = ACT_MAP.get(kpi, "-")
                pa_h += '<tr><td>%s</td><td>%s</td><td style="text-align:left;font-weight:400;font-size:11px">%s</td><td style="font-weight:900;color:#dc2626;font-size:14px">%d</td></tr>' % (kpi, resp, action, total)
            pa_h += '</tbody></table>'
            st.markdown(pa_h, unsafe_allow_html=True)
            
            # Détails par KPI avec expanders pour voir les OT/Avis
            st.markdown('<div class="stl" style="margin-top:16px">Détail des Anomalies par Indicateur</div>', unsafe_allow_html=True)
            
            for kpi, total, pdets in kpi_ano_summary:
                with st.expander("🔴 %s — %d anomalie(s)" % (kpi, total), expanded=False):
                    resp = KPI_RESP_MAP.get(kpi, "-")
                    action = ACT_MAP.get(kpi, "-")
                    st.markdown("**Responsable :** %s  |  **Action :** %s" % (resp, action))
                    
                    for poste, cnt in pdets:
                        items = ano_details.get((poste, kpi), [])
                        item_type = "Avis" if "avis" in kpi.lower() or kpi == "appel avis approuvé" else "OT"
                        if items:
                            st.markdown("**%s** (%d %s(s)) :" % (poste, cnt, item_type))
                            # Afficher les OT/Avis sous forme de tags
                            tags_html = '<div class="ano-detail-box"><div class="ano-list">'
                            for it in items:
                                tags_html += '<span class="ano-item">%s</span>' % str(it)
                            tags_html += '</div></div>'
                            st.markdown(tags_html, unsafe_allow_html=True)
                        else:
                            st.markdown("**%s** : 0 anomalie" % poste)

    # -----------------------------------------------------------
    # TAB 5 : ANOMALIES
    # -----------------------------------------------------------
    with tab5:
        st.markdown('<div class="stl">Tableau des Anomalies par Poste et Indicateur</div>', unsafe_allow_html=True)
        st.markdown('<p style="font-size:13px;color:#64748b;margin-bottom:12px">Nombre d\'OT ou Avis à corriger pour atteindre 100%% pour chaque indicateur.</p>', unsafe_allow_html=True)
        
        # Construire le tableau des anomalies
        ano_cols = ["Poste de travail"] + ALL_KPI + ["Total"]
        ano_h = '<table class="tw at"><thead><tr>'
        for c in ano_cols:
            ano_h += '<th>%s</th>' % c
        ano_h += '</tr></thead><tbody>'
        
        for poste in posts:
            ano_h += '<tr><td style="font-weight:700">%s</td>' % poste
            row_total = 0
            for kpi in ALL_KPI:
                cnt = ano_counts.get((poste, kpi), 0)
                row_total += cnt
                if cnt == 0:
                    ano_h += '<td style="text-align:center;background:#d1fae5;color:#065f46;font-weight:600">0</td>'
                else:
                    ano_h += '<td style="text-align:center;background:#fee2e2;color:#991b1b;font-weight:900;font-size:14px">%d</td>' % cnt
            # Total par ligne
            if row_total == 0:
                ano_h += '<td style="text-align:center;background:#d1fae5;color:#065f46;font-weight:700">0</td>'
            else:
                ano_h += '<td style="text-align:center;background:#fee2e2;color:#991b1b;font-weight:900;font-size:14px">%d</td>' % row_total
            ano_h += '</tr>'
        
        # Ligne total par colonne
        ano_h += '<tr><td style="font-weight:900;background:#1e3a5f!important;color:#fff!important">Total</td>'
        grand_total = 0
        for kpi in ALL_KPI:
            col_total = sum(ano_counts.get((p, kpi), 0) for p in posts)
            grand_total += col_total
            if col_total == 0:
                ano_h += '<td style="text-align:center;background:#d1fae5;color:#065f46;font-weight:700">0</td>'
            else:
                ano_h += '<td style="text-align:center;background:#fee2e2;color:#fff;font-weight:900;font-size:14px">%d</td>' % col_total
        if grand_total == 0:
            ano_h += '<td style="text-align:center;background:#d1fae5;color:#065f46;font-weight:900">0</td>'
        else:
            ano_h += '<td style="text-align:center;background:#ef4444;color:#fff;font-weight:900;font-size:15px">%d</td>' % grand_total
        ano_h += '</tr></tbody></table>'
        st.markdown(ano_h, unsafe_allow_html=True)
        
        # Détail cliquable des anomalies
        st.markdown('<div class="stl" style="margin-top:16px">Détail des Anomalies (cliquez pour développer)</div>', unsafe_allow_html=True)
        
        for kpi in ALL_KPI:
            kpi_total = sum(ano_counts.get((p, kpi), 0) for p in posts)
            if kpi_total > 0:
                with st.expander("🔴 %s — %d anomalie(s) au total" % (kpi, kpi_total)):
                    for poste in posts:
                        cnt = ano_counts.get((poste, kpi), 0)
                        if cnt > 0:
                            items = ano_details.get((poste, kpi), [])
                            item_type = "Avis" if "avis" in kpi.lower() or kpi == "appel avis approuvé" else "OT"
                            with st.expander("%s : %d %s(s)" % (poste, cnt, item_type)):
                                if items:
                                    tags_html = '<div class="ano-detail-box"><div class="ano-list">'
                                    for it in items:
                                        tags_html += '<span class="ano-item">%s</span>' % str(it)
                                    ...
                                    st.markdown("Aucun détail disponible.")
    ...
    if st.session_state.get("do_save", False):
        ...
        for poste in posts:
            ...
            green_cnt = 0
            for kpi in QK:
                v = round(ckdf.loc[poste, kpi], 2)
                row[kpi] = v
                if get_cell_color(kpi, v) in ["green", "yellow"]:
                    green_cnt += 1
            row["Score Performance"] = round((green_cnt / len(QK)) * 100, 2) if QK else 0
            prows.append(row)
        ...
        for poste in posts:
            ...
            for kpi in PK:
                ...
                if get_cell_color(kpi, v) in ["green", "yellow"]:
                    green_cnt += 1
            row["Score Qualité"] = round((green_cnt / len(PK)) * 100, 2) if PK else 0
            qrows.append(row)
        ...
        save_kpis_to_excel(prows, pcols, qrows, qcols, ano_p_rows, ano_p_cols, ano_q_rows, ano_q_cols, date_input)
        st.success("✅ KPIs sauvegardés dans kpis/indicateurs_kpis.xlsx")

    # ============================================================
    # FOOTER
    # ============================================================
    st.markdown('<div class="footer">Dashboard KPI Maintenance — Données au %s — %d postes — %d indicateurs</div>' % (date_input, len(posts), len(ALL_KPI)), unsafe_allow_html=True)
