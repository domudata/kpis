...
def make_sparkline_svg(values, width=90, height=30, color="#3b82f6", stroke_w=1.8):
    """Sparkline SVG avec gradient unique (pas de conflit d'ID)."""
...
def _spark_pivot(hist_df, kpi_name, section):
    """Pivot : index=Poste, colonnes=Date_str, valeurs=KPI."""
    if hist_df.empty:
        return pd.DataFrame()
    ...
    pv = sub.pivot_table(index="Poste de travail", columns="Date_str",
                         values=kpi_name, aggfunc="first")
    return pv.sort_index(axis=1)


def render_sparkline_table(hist_df, kpi_list, section_label, table_css):
    """Tableau global : 1 ligne par poste, 1 colonne par KPI avec sparkline."""
    ...
    pivots = {}
    for kpi in kpi_list:
        pv = _spark_pivot(hist_df, kpi, sec_key)
        if not pv.empty:
            pivots[kpi] = pv
    if not pivots:
    ...
    for poste in postes:
        ...
        for kpi in kpi_list:
            ...
            if kpi in LOWER_BETTER:
                clr = "#10b981" if last is not None and last <= cible else "#ef4444"
            else:
                clr = "#10b981" if last is not None and last >= cible else "#ef4444"

            svg = make_sparkline_svg(vals, color=clr)

            # Variation 1ère → dernière
            var_html = ""
