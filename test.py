...
def page_suivi_evaluation(posts, hist_df, var_df):
    ...
    .poste-header {
        ...
        color: #fff; padding: 10px 18px; border-radius: 10px 10px 0 0;
        font-weight: 800; font-size: 14px; display: flex; align-items: center;
        gap: 10px; letter-spacing: 0.3px; box-shadow: 0 2px 8px rgba(30,58,95,0.15);
    }
    .poste-header .badge {
        background: rgba(255,255,255,0.18); padding: 3px 12px; border-radius: 5px;
        font-size: 10px; letter-spacing: 1.5px; text-transform: uppercase; font-weight: 700;
    }
    .chart-label {
        text-align: center; font-weight: 800; font-size: 12px; padding: 7px 10px;
        border-radius: 8px 8px 0 0; letter-spacing: 0.5px; text-transform: uppercase;
    }
    ...
    .chart-footer { text-align: center; font-size: 11px; padding: 5px 8px; border-radius: 0 0 8px 8px; }
    ...
    .synth-card {
        ...
        padding: 16px; margin: 10px 0; box-shadow: 0 2px 8px rgba(0,0,0,0.04); overflow-x: auto;
    }
    .synth-card h3 { font-size: 15px; font-weight: 800; margin: 0 0 12px 0; padding-bottom: 8px; border-bottom: 2px solid #e2e8f0; }
    .synth-tbl-detailed { width: 100%; border-collapse: collapse; font-family: 'Inter', sans-serif; font-size: 11px; }
    .synth-tbl-detailed thead th {
        background: #1e3a5f; color: #fff; font-weight: 700; font-size: 10px; padding: 8px 6px;
        border: 1px solid #1e3a5f; white-space: nowrap; text-transform: uppercase;
        letter-spacing: 0.3px; position: sticky; top: 0; z-index: 10;
    }
    .synth-tbl-detailed thead th:first-child { position: sticky; left: 0; z-index: 11; min-width: 140px; }
    .synth-tbl-detailed tbody td {
        padding: 6px 5px; border: 1px solid #e2e8f0; text-align: center;
        vertical-align: middle; color: #1e293b !important;
    }
    .synth-tbl-detailed tbody td:first-child {
        position: sticky; left: 0; background: #fff !important; z-index: 5;
        font-weight: 800; text-align: left; white-space: nowrap; border-right: 2px solid #e2e8f0;
    }
    ...
    .kpi-summary-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 8px; margin: 12px 0; }
    .kpi-summary-card { border-radius: 8px; padding: 10px 12px; text-align: center; border: 1px solid; }
    ...
    for idx, poste in enumerate(posts):
        ...
        st.markdown(
            f"""<div class="poste-header" style="border-left:5px solid {accent};">
                <span class="badge">Poste</span>{poste}</div>""", unsafe_allow_html=True)
        ...
        with c1:
            ...
            if not p_data.empty and "Score Performance" in p_data.columns:
                ...
                last_v = vals[-1]; prev_v = vals[-2] if len(vals) > 1 else last_v
                diff = last_v - prev_v; is_up = diff >= 0
                trend_icon = "▲" if is_up else "▼"; trend_clr = "#059669" if is_up else "#ef4444"
                fig_p = go.Figure()
                fig_p.add_trace(go.Scatter(x=p_data["Date_parsed"], y=p_data["Score Performance"],
                    fill='tozeroy', fillcolor='rgba(5,150,105,0.07)', line=dict(width=0), showlegend=False, hoverinfo='skip'))
                fig_p.add_trace(go.Scatter(x=p_data["Date_parsed"], y=p_data["Score Performance"],
                    mode='lines+markers', name='Performance', line=dict(color='#059669', width=3, shape='spline'),
                ...
                fig_p.update_layout(margin=dict(t=8, b=32, l=48, r=16), height=210,
                ...
                st.markdown(f"""<div class="chart-footer perf">
                    ...
                    <span style="color:{trend_clr};margin-left:4px;font-weight:700;">({diff:+.1f})</span></div>""", unsafe_allow_html=True)
            else:
        ...
        with c2:
            ...
            if not q_data.empty and "Score Qualite" in q_data.columns:
                ...
                last_v = vals[-1]; prev_v = vals[-2] if len(vals) > 1 else last_v
                diff = last_v - prev_v; is_up = diff >= 0
                trend_icon = "▲" if is_up else "▼"; trend_clr = "#2563eb" if is_up else "#ef4444"
                fig_q = go.Figure()
                fig_q.add_trace(go.Scatter(x=q_data["Date_parsed"], y=q_data["Score Qualite"],
                    fill='tozeroy', fillcolor='rgba(37,99,235,0.07)', line=dict(width=0), showlegend=False, hoverinfo='skip'))
                fig_q.add_trace(go.Scatter(x=q_data["Date_parsed"], y=q_data["Score Qualite"],
                    mode='lines+markers', name='Qualité', line=dict(color='#2563eb', width=3, shape='spline'),
                ...
                fig_q.add_hline(y=85, line_dash="dash", line_color="#ef4444", line_width=1.5,
                    annotation_text="Cible 85%", annotation_position="top right",
                    annotation_font=dict(size=9, color="#ef4444", family="Inter"))
                fig_q.update_layout(margin=dict(t=8, b=32, l=48, r=16), height=210,
                    yaxis=dict(range=[0, 110], dtick=20, title=dict(text="%", font=dict(size=10, color="#64748b"))),
                    xaxis=dict(showgrid=True, gridcolor='#f1f5f9', tickfont=dict(size=9, color="#64748b")),
                    showlegend=False, plot_bgcolor='#fafbfc', paper_bgcolor='white')
                st.plotly_chart(fig_q, use_container_width=True, config={"displayModeBar": False})
                st.markdown(f"""<div class="chart-footer qual">
                    <span style="color:{trend_clr};font-weight:900;font-size:14px;">{trend_icon} {last_v:.1f}%</span>
                    <span style="color:#64748b;margin-left:8px;">vs {prev_v:.1f}% préc.</span>
                    <span style="color:{trend_clr};margin-left:4px;font-weight:700;">({diff:+.1f})</span></div>""", unsafe_allow_html=True)
            else:
    ...
    with sel_c1:
        ...
        date_from = st.selectbox("📅 Date de début", dates_labels, index=idx_from, key="synth_date_from", format_func=lambda x: f"Période : {x}")
    with sel_c2:
        date_to = st.selectbox("📅 Date de fin", dates_labels, index=len(dates_labels) - 1, key="synth_date_to", format_func=lambda x: f"Période : {x}")

    idx_from_val = dates_labels.index(date_from); idx_to_val = dates_labels.index(date_to)
...
def _render_synthesis_detail(hist_section, date_from, date_to, posts,
    ...
    nb_improv = nb_degrad = nb_stable = 0; total_diff_score = 0; nb_postes_score = 0
    for poste in posts:
    ...
    st.markdown(
        f"""<div style="display:flex;gap:20px;flex-wrap:wrap;padding:8px 0;font-size:12px;font-weight:600;">
            <span style="display:flex;align-items:center;gap:5px;">
                <span style="display:inline-block;width:14px;height:14px;background:#d1fae5;border-radius:3px;border:1px solid #a7f3d0;"></span>Amélioration</span>
            <span style="display:flex;align-items:center;gap:5px;">
                <span style="display:inline-block;width:14px;height:14px;background:#fee2e2;border-radius:3px;border:1px solid #fca5a5;"></span>Dégradation</span>
            <span style="display:flex;align-items:center;gap:5px;">
                <span style="display:inline-block;width:14px;height:14px;background:#f8fafc;border-radius:3px;border:1px solid #e2e8f0;"></span>Stable (écart ≤ 0.5 pt)</span>
        </div>""", unsafe_allow_html=True)
...
def inject_custom_css():
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    div[data-testid="stSidebar"] .stSelectbox label,div[data-testid="stSidebar"] .stMultiSelect label{color:rgba(255,255,255,.9)!important;font-weight:600;font-size:13px;text-transform:uppercase;letter-spacing:.5px}
    ...
    div[data-testid="stSidebar"] .stSelectbox>div>div,div[data-testid="stSidebar"] .stMultiSelect>div>div{background:rgba(255,255,255,.95)!important;border-radius:5px}
...
def main():
    ...
    if not os.path.exists(OT_FILE) or not os.path.exists(AV_FILE):
        st.markdown(f"""<div style="min-height:60vh;display:flex;flex-direction:column;align-items:center;justify-content:center;">
            <div style="font-size:80px;margin-bottom:20px;">📂</div>
            <h2 style="color:#1e3a5f;font-weight:800;">Fichiers introuvables</h2>
            <p style="color:#64748b;font-size:16px;margin-top:8px;">
                Veuillez placer <strong>ot.xlsx</strong> et <strong>avis.xlsx</strong> dans le même dossier que ce script.
            </p>
            <div style="margin-top:16px;font-size:13px;color:#94a3b8;">
                Dossier actuel : <code style="background:#f1f5f9;padding:4px 8px;border-radius:4px;">{os.getcwd()}</code>
            </div>
        </div>""", unsafe_allow_html=True)
        return
    ...
    with st.sidebar:
        ...
        if logo_b64:
            st.markdown(f'<img src="data:image/png;base64,{logo_b64}" style="width:100%;max-width:180px;margin:0 auto 10px;display:block;border-radius:6px;">', unsafe_allow_html=True)
        st.markdown("### 📁 Fichiers chargés")
        st.markdown(f"""<div style="background:rgba(255,255,255,0.1);padding:10px 12px;border-radius:6px;border:1px solid rgba(255,255,255,0.15);margin-bottom:8px;">
            <div style="font-size:12px;font-weight:700;color:rgba(255,255,255,0.8);margin-bottom:4px;">📄 OT</div>
            <div style="font-size:11px;color:rgba(255,255,255,0.6);">{OT_FILE} — {len(df)} lignes</div>
        </div>""", unsafe_allow_html=True)
        st.markdown(f"""<div style="background:rgba(255,255,255,0.1);padding:10px 12px;border-radius:6px;border:1px solid rgba(255,255,255,0.15);margin-bottom:8px;">
            <div style="font-size:12px;font-weight:700;color:rgba(255,255,255,0.8);margin-bottom:4px;">📄 Avis</div>
            <div style="font-size:11px;color:rgba(255,255,255,0.6);">{AV_FILE} — {len(avf)} lignes</div>
        </div>""", unsafe_allow_html=True)
        st.markdown("---")
        poste_filter = st.multiselect("🏭 Postes", options=apm, key="poste_filt")
        st.markdown("---")
    ...
    with tab_suivi_kpi:
        ...
        if sel_poste:
            ...
            for kpi in QK:
                val = p_vals.get(kpi, 0); cible = CIBLE.get(kpi, 100)
                pct = min(val, 100)
                ...
                st.markdown(f"""<div class="ca"><div class="ct">{kpi}</div>
                    <div class="car"><div class="cal">{kpi}</div>
                    ...
                    <div class="cav-tgt">Cible: {cible}%</div></div></div>""", unsafe_allow_html=True)
            ...
            for kpi in PK:
                val = q_vals.get(kpi, 0); cible = CIBLE.get(kpi, 100)
                pct = min(val, 100)
    ...
    st.markdown(f'<div class="footer">Dashboard KPI Maintenance — Suivi & Évaluation — {fichier_date}</div>', unsafe_allow_html=True)
