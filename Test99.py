...
# ============================================================
# NOUVELLE FONCTION POUR CALCULER LES ANOMALIES
# ============================================================
def calculer_anomalies(ckdf, posts, categorie="Tous"):
    if categorie == "Performance": kpis_filtrés = QK
    elif categorie == "Qualité": kpis_filtrés = PK
    else: kpis_filtrés = QK + PK
    
    anomaly_records = []
    for poste in posts:
        if poste not in ckdf.index: continue
        for kpi in kpis_filtrés:
            if kpi not in ckdf.columns or kpi not in CIBLE: continue
            valeur = ckdf.loc[poste, kpi]
            cible = CIBLE[kpi]
            if pd.isna(valeur): continue
            
            is_anomaly = False
            ecart = float(valeur) - float(cible)
            
            if kpi in LOWER_BETTER:
                if valeur > cible: is_anomaly = True
            else:
                if valeur < cible: is_anomaly = True
            ...
            if is_anomaly:
                anomaly_records.append({
                    ...
                    "Valeur": round(float(valeur), 2), "Cible": cible,
                    "Écart %": round((ecart / cible * 100), 2) if cible != 0 else 0,
                    "Responsable": KPI_RESP_MAP.get(kpi, "N/A"),
                    "Action": ACT_MAP.get(kpi, "N/A")
                })
    
    df_ano = pd.DataFrame(anomaly_records)
    dict_poste = df_ano.groupby("Poste de travail").size().to_dict() if not df_ano.empty else {}
    dict_kpi = df_ano.groupby("KPI").size().to_dict() if not df_ano.empty else {}
    
    return len(df_ano), df_ano, dict_poste, dict_kpi
...
def main():
    ...
    def show_pie_pair(piv_df, title_prefix):
        ...
        fig.add_trace(go.Pie(labels=gc.index, values=gc.values, hole=0.4, textinfo='percent+label', texttemplate='%{label}<br>%{percent:.1%}<br>(%{value})', textposition='inside', insidetextorientation='radial', textfont=dict(size=14, color='white', family='Inter, sans-serif'), marker=dict(colors=colors, line=dict(color='#FFFFFF', width=3))), 1, 1)
        p2 = pd.Series([r, nr], index=["Réalisés (CLOT+TCLO)", "Non Réalisés"])
        fig.add_trace(go.Pie(labels=p2.index, values=p2.values, hole=0.5, textinfo='percent+label', texttemplate='%{label}<br>%{percent:.1%}<br>(%{value})', textposition='inside', insidetextorientation='radial', textfont=dict(size=14, color='white', family='Inter, sans-serif'), marker=dict(colors=["#10b981", "#8b5cf6"], line=dict(color='#FFFFFF', width=3))), 1, 2)
    ...
    def show_simple_pie(piv_df, title, keep_non_carac=False):
        ...
        for c in counts.index:
            cs = str(c)
            if cs in cm: clrs.append(cm[cs])
            else: clrs.append(tp[pi % len(tp)]); pi += 1
        ts = counts.sum(); pl = [0.05 if (v/ts)*100 < 10 else 0 for v in counts.values]
        fig = go.Figure(go.Pie(labels=counts.index, values=counts.values, hole=0.4, sort=False, textinfo="percent", textposition="outside", pull=pl, marker=dict(colors=clrs, line=dict(color="white", width=2))))
    ...
    def calc_kpis(df_i, av_i, now_ts, posts):
        ...
        }).reindex(posts, fill_value=100.0)
        return res

    # ====================================================================
    # CHARGEMENT AUTOMATIQUE DES FICHIERS LOCAUX
    # ====================================================================
    try:
        with open("ot.xlsx", "rb") as f: ot_bytes = f.read()
        with open("avis.xlsx", "rb") as f: av_bytes = f.read()
    except FileNotFoundError:
        st.error("Fichiers introuvables. Assurez-vous que 'ot.xlsx' et 'avis.xlsx' sont dans le même dossier que l'application.")
        st.stop()

    df, avf, apm, now_ts = prepare_data(ot_bytes, av_bytes, fichier_date)
    if not apm:
        st.warning("Aucun poste de travail trouvé (SF1/SF2)."); st.stop()

    # ====================================================================
    # CALCUL DES KPIs ET ANOMALIES
    # ====================================================================
    res = calc_kpis(df, avf, now_ts, apm)
    ckdf = res['ckdf']

    # Calcul des anomalies
    nb_ano_perf, df_ano_perf, ano_perf_poste, ano_perf_kpi = calculer_anomalies(ckdf, apm, categorie="Performance")
    nb_ano_qual, df_ano_qual, ano_qual_poste, ano_qual_kpi = calculer_anomalies(ckdf, apm, categorie="Qualité")

    # Préparation pour export Excel
    cols_excel = ["Poste de travail", "KPI", "Valeur", "Cible", "Écart %", "Responsable", "Action"]
    if not df_ano_perf.empty: ano_p_c = cols_excel; ano_p_r = df_ano_perf[cols_excel].to_dict('records')
    else: ano_p_r, ano_p_c = [], []
    if not df_ano_qual.empty: ano_q_c = cols_excel; ano_q_r = df_ano_qual[cols_excel].to_dict('records')
    else: ano_q_r, ano_q_c = [], []

    # ====================================================================
    # INTERFACE UTILISATEUR (HEADER, CARDS, SIDEBAR)
    # ====================================================================
    logo_b64 = get_logo_base64()
    logo_html = f'<img src="data:image/png;base64,{logo_b64}" class="logo">' if logo_b64 else ""
    st.markdown(f'<div class="mh">{logo_html}<h1>DASHBOARD KPI</h1><div class="db">{fichier_date}</div></div>', unsafe_allow_html=True)

    # Affichage des cartes d'anomalies
    st.markdown(f"""<div class="cr">
        <div class="cc c4"><div class="cv">{nb_ano_perf}</div><div class="cl">Anomalies Performance</div></div>
        <div class="cc c7"><div class="cv">{nb_ano_qual}</div><div class="cl">Anomalies Qualité</div></div>
    </div>""", unsafe_allow_html=True)

    # Filtre Sidebar
    st.sidebar.markdown("---")
    filtre_ano = st.sidebar.selectbox("Détail Anomalies", ["Aucun", "Performance", "Qualité"], index=0)
    if filtre_ano == "Performance" and not df_ano_perf.empty:
        st.markdown('<div class="stl">❌ Détail Anomalies Performance</div>', unsafe_allow_html=True)
        st.dataframe(df_ano_perf.sort_values("Écart %", ascending=False), hide_index=True, use_container_width=True)
    elif filtre_ano == "Qualité" and not df_ano_qual.empty:
        st.markdown('<div class="stl">❌ Détail Anomalies Qualité</div>', unsafe_allow_html=True)
        st.dataframe(df_ano_qual.sort_values("Écart %", ascending=False), hide_index=True, use_container_width=True)

    # (Suite logique de votre code : Onglets, Graphiques, Export Excel...)
    # Exemple de base pour que le fichier tourne :
    tab1, tab2, tab3 = st.tabs(["📊 Performance", "🎯 Qualité", "📁 Export"])
    with tab1:
        st.markdown("Affichez ici vos graphiques de performance en utilisant ckdf, res['dfp']...")
    with tab2:
        st.markdown("Affichez ici vos graphiques de qualité...")
    with tab3:
        if st.button("Générer le fichier Excel"):
            # Vous pouvez appeler votre fonction save_kpis_to_excel ici
            st.success("Fichier Excel généré avec succès dans le dossier /kpis !")
